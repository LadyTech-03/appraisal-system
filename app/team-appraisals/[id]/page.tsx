...
export default function TeamAppraisalDetailPage() {
  ...
  useEffect(() => {
    ...
    if (appraisal.appraiserId !== user?.id) {
      router.push("/dashboard")
      return
    }
  ...
  const canEdit = appraisal.status === "draft" || appraisal.status === "submitted"
        ...
        <main className="flex-1 p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-primary">{employee?.name}'s Appraisal</h1>
            ...
            <div className="flex items-center space-x-3">
              ...
              {canEdit && (
                <Button onClick={() => router.push(`/team-appraisals/${appraisal.id}/edit`)}>
          ...
          {/* Appraisal Details */}
          {employee && appraiser && <AppraisalView appraisal={appraisal} employee={employee} appraiser={appraiser} />}
        </main>
      </div>
    </div>
  )
